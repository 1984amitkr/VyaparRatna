...
VARNADIPANCHAK_GRID = {
    ...
    (6, 2): "Li",   (6, 3): "Vi", (6, 4): "Le", (6, 5): "ॡ ḹ", (6, 6): "ए e"
}
...
# Reset to Current Time Button Logic
if st.sidebar.button("🔄 Reset to Current Time", use_container_width=True):
    now_live = datetime.datetime.now(MUMBAI_TZ)
    st.session_state.mode = "LIVE"
    st.session_state.hist_date = now_live.date()
    st.session_state.hist_time = now_live.time()
    st.rerun()
...
# -------------------------------------------------------------------
# 8. AUTO-REFRESH SCRIPT FOR LIVE MODE ONLY (Every 5 mins / 300,000 ms)
# -------------------------------------------------------------------
if st.session_state.mode == "LIVE":
...
if btn_c1.button("Show All Planets", use_container_width=True):
    st.session_state["filter_multiselect"] = all_planets
if btn_c2.button("Malefics Only", use_container_width=True):
    st.session_state["filter_multiselect"] = MALEFICS
if btn_c3.button("Benefics Only", use_container_width=True):
    st.session_state["filter_multiselect"] = BENEFICS
if btn_c4.button("Clear Rays", use_container_width=True):
    st.session_state["filter_multiselect"] = []

selected_planets = st.multiselect(
    "Filter Active Vedha Rays:",
    options=all_planets,
    key="filter_multiselect"
)

sbc_html = render_sbc_grid_visual_with_svg(data, selected_planets, effective_datetime)
st.components.v1.html(sbc_html, height=720, scrolling=False)

st.markdown("---")

# --- FIFTH SECTION: EPHEMERIS & VEDHA DATA TABLE ---
st.markdown("### 🪐 Planetary Motion & Detailed Vedha Targets")
df_data = pd.DataFrame(data)
st.dataframe(
    df_data[["Planet", "Nakshatra", "Pada", "Longitude", "Speed (°/day)", "Motion", "Primary Vedha", "Front Target", "Left Target", "Right Target"]],
    use_container_width=True,
    hide_index=True
)
