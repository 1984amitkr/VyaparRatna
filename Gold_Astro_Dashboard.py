...
st.set_page_config(
    page_title="Vyapar Ratna - Gold Astro Engine V2 (Ephem Engine)",
    page_icon="🪙",
    layout="wide"
)

# --- ASTRONOMICAL CONSTANTS & VEDIC UTILITIES ---
ZODIAC_SIGNS = [
...
def get_lahiri_ayanamsha(ephem_date):
    """Approximate Lahiri Ayanamsha for Sidereal conversion."""
    year = ephem_date.datetime().year + (ephem_date.datetime().month / 12.0)
    ayanamsha = 23.85 + (year - 2000) * (50.29 / 3600.0)
    return ayanamsha

def get_ephem_positions(calc_date):
    """Calculate planetary positions in Sidereal coordinates using PyEphem."""
    date_str = calc_date.strftime("%Y/%m/%d 12:00:00")
    e_date = ephem.Date(date_str)
    ayanamsha = get_lahiri_ayanamsha(e_date)
    ...
    for name, body in bodies.items():
        ...
        tropical_lon = float(ecl_lon) * 180.0 / np.pi
        sidereal_lon = (tropical_lon - ayanamsha) % 360.0

        # Motion calculation (delta over 1 hour)
        e_date_next = ephem.Date(e_date + ephem.hour)
        body.compute(e_date_next)
        ecl_lon_next = ephem.Ecliptic(body).lon
        tropical_lon_next = float(ecl_lon_next) * 180.0 / np.pi
        
        # Check Retrograde status
        speed = tropical_lon_next - tropical_lon
        is_retrograde = speed < 0 if name not in ["Sun", "Moon"] else False
    ...
    # Derive Rahu (Mean North Node) approximation
    rahu_tropical = (125.044555 - 0.0529538083 * (e_date - ephem.Date("1900/01/01 12:00:00"))) % 360.0
    rahu_sidereal = (rahu_tropical - ayanamsha) % 360.0
    rahu_sign_idx = int(rahu_sidereal // 30)
    rahu_nak_idx = int(rahu_sidereal // (360.0 / 27.0))

    positions["Rahu"] = {
        "longitude": rahu_sidereal,
        "sign": ZODIAC_SIGNS[rahu_sign_idx],
        "nakshatra": NAKSHATRAS[rahu_nak_idx],
        "is_retrograde": True
    }
...
st.caption("Ephem Engine Version | Multi-Layer Planetary Scoring & Price Filter Architecture")

st.sidebar.header("🗓️ Calculation Controls")
calc_date = st.sidebar.date_input("Select Evaluation Date", datetime.date.today())

positions = get_ephem_positions(calc_date)
...
if positions:
    ...
    st.subheader("🪐 Live Sidereal Planetary Positions (PyEphem Engine)")
    eph_data = []
